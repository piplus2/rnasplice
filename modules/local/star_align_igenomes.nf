process STAR_ALIGN_IGENOMES {
    ...
    def seq_center_arg = seq_center ? "--outSAMattrRGline ID:${prefix} 'CN:${seq_center}' 'SM:${prefix}' ${seq_platform_str} " : "--outSAMattrRGline ID:${prefix} 'SM:${prefix}' ${seq_platform_str} "
    def out_sam_type = (args.contains('--outSAMtype')) ? '' : '--outSAMtype BAM Unsorted'
    def mv_unsorted_bam_command = (args.contains('--outSAMtype BAM Unsorted SortedByCoordinate')) ? "mv ${prefix}.Aligned.out.bam ${prefix}.Aligned.unsort.out.bam" : ''
    """
    STAR \\
        --genomeDir ${index} \\
        --readFilesIn ${reads}  \\
